    ...
    def request_preview(self) -> None:
        ...
        if self._view_model.time_range.preset == TimeRangePreset.CUSTOM.value:
            ...
            if config.start_time >= config.end_time:
                return
        is_tick_mode = config.execution_mode == BacktestExecutionMode.HISTORICAL_TICK
        if is_tick_mode and config.start_time is None:
        ...
        if tick_mode_range_too_wide(is_tick_mode, config.start_time, config.end_time):
            ...
            # range as an unbounded one, and this coordinator's own
            # coverage dispatch below has no progress bar or cancellation
