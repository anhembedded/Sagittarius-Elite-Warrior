...
# BUG-051's regression test (`test_status_rows_are_not_clipped_to_one_line_of_text`)
# was removed under `EPIC-015` Phase 2: the status table is `DatabaseStatusPanel`
# (a QML `ListView` over `DatabaseStatusRow.qml`) now, not `QListView.setIndexWidget()`
# over `_StatusRowWidget`. BUG-051's actual mechanism — `setIndexWidget()` not
# reporting the placed widget's real height back to the list, so the delegate
# clips it to one line — cannot recur here: a Qt Quick `ListView` sizes each
# delegate from the delegate's own `implicitHeight`
# (`DatabaseStatusRow.qml: implicitHeight: 40`), there is no separate "list
