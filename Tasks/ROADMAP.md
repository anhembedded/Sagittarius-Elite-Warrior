...
- [x] **`BUG-112`**: [Cài mới (fresh install): combo khung thời gian trông như đã chọn nhưng `liveInterval` rỗng, "Nạp chiến lược" bị từ chối câm lặng ở lần bấm đầu tiên. Phát hiện lúc xây `EPIC-023C` — test tích hợp đầu tiên bấm Arm thật qua config store thật chưa từng lưu. `LiveStrategyConfigStore.load()` trả `interval=""` khi chưa lưu gì; `StrategyArmingCoordinator.restore_into_view_model()` truyền thẳng giá trị đó, không có fallback như `saved_key` đã có, trong khi combo tự chọn index 0 im lặng qua `QComboBox.addItems()` — combo nhìn như đã chọn, `liveInterval` vẫn rỗng, `build_config()` bị `ArmStrategyCommand` từ chối. Sửa 1 chỗ dùng chung cho cả màn Giao dịch lẫn Dev Board: fallback về `interval_options[0]` giống hệt cách `saved_key` đã fallback. *(Đổi số từ `BUG-111` — trùng số với một phiên song song, đóng cùng lúc, vụ Binance clock-skew)*](bug_report/completed/BUG-112_fresh_install_arm_refused_because_liveinterval_stays_empty.md)
