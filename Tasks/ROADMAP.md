...
- [x] **`BUG-107`**: [Nút "Nạp chiến lược" (`EPIC-022D`) `AttributeError` mọi lần bấm — `on_arm_clicked()` gọi `ActionOwnershipTracker.start_action()`, một method chưa từng tồn tại (API thật: `begin_action(kind, config, previous_state)`, và nó không bao giờ trả `None` nên nhánh `if action is None: return` đi kèm cũng là code chết). Lỗi ném ra trước cả dòng logic đầu tiên nên không `ArmStrategyCommand` nào được gửi. Lọt qua cả ba cổng vì `mypy` loại toàn bộ `src/presentation/`, `ruff` không phân tích liên-module, và 12 test của card đều gọi `arm()` — method nằm **dưới** nút — chứ chưa test nào chạm entry point `on_arm_clicked()` hay chuỗi tín hiệu `requestArm()` → `armRequested` → `_on_arm_requested`. Sửa 1 dòng + 5 regression test đi đúng đường của nút](bug_report/completed/BUG-107_arm_strategy_button_calls_a_tracker_method_that_never_existed.md)
