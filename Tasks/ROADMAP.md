...
- [x] **`BUG-115`**: [Đặt lệnh Limit thủ công luôn bị từ chối: `LIMIT order is missing time_in_force.` User tự chạy app thật, click "Limit" trên thẻ đặt lệnh thủ công (`EPIC-024B`), gửi log lỗi thật. Root cause: `map_order_to_futures_params()` có sẵn guard đúng, từ chối LIMIT thiếu `time_in_force` — nhưng `PreviewOrderQueryHandler.execute()` chưa từng gán field đó, latent từ `EPIC-021E` vì nhánh chiến lược tự động chỉ gửi MARKET; thẻ đặt lệnh thủ công là caller LIMIT thật đầu tiên chạm guard. Sửa: gán `time_in_force=TimeInForce.GTC` khi `order_type is LIMIT`, giữ `None` cho MARKET. 2 test mới, xác nhận đỏ đúng lý do (khớp nguyên văn lỗi thật) trước khi sửa](bug_report/completed/BUG-115_limit_order_missing_time_in_force.md)
- [x] **`BUG-114`**: [`GetOpenPositionsQuery`/`EnableTradingCommand` crash trên tài khoản có vị thế thật: `/fapi/v3/positionRisk` không có field `leverage`/`marginType` như code giả định. User tự chạy app thật, gửi log lỗi thật + ảnh Binance UI xác nhận vị thế 20x thật. Root cause: mapper đọc thẳng 2 field không tồn tại trên response V3 thật (viết theo tài liệu, chưa xác minh lệnh gọi thật). Sửa: suy `leverage` từ `notional/initialMargin` (công thức margin của Binance, verify khớp đúng 20x trên payload thật), `margin_type` từ `isolatedMargin`. 1 test mới dùng đúng payload thật làm fixture, mutation-verify tái hiện đúng lỗi trước khi sửa](bug_report/completed/BUG-114_position_leverage_field_missing_from_real_v3_payload.md)
