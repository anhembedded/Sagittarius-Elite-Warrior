...
| 🔴 **Backlog** | 52 | 27.7% |
| ❌ **Cancelled** | 6 | 3.2% |
| 📈 **Tổng số Task** | **187** | **100%** |

> 🐞 **Lỗi (bug) không tính trong bảng trên** — theo dõi riêng ở [Bug Board](bug_report/README.md), nơi liệt kê cả bug **đang mở** lẫn đã sửa.

> **Cập nhật 2026-09-10:** Hoàn thành `BOT-132` (`QuickSurface`) — đóng `BUG-115`, và đóng nốt nửa còn lại của `BUG-102`. Kèm `TASK-042` bên repo Engine (`create_quick_widget(background=...)`), 2 commit riêng theo `ONBOARDING.md` §2. Đối soát lại số theo thư mục thật: completed 129, backlog 52, cancelled 6.
>
...
| **[EPIC-002](epics/EPIC-002_static_type_checking_in_local_ci/README.md)** | Kiểm tra kiểu tĩnh (`mypy`) trong CI cục bộ | ✅ Hoàn thành (5/5 task con) — 2026-09-08 |
| **[EPIC-003](epics/EPIC-003_presenter_and_god_file_decomposition/README.md)** | Phân rã Presenter/File quá tải (Coordinator Pattern, Domain Policy, QML) | 🟡 Đang làm (5/8 xong, 1 huỷ — `003D`; `003F1` xong 02/09, lát cắt trade log — 5 lát còn lại) |
| **[EPIC-004](epics/EPIC-004_static_security_and_quality_analysis/README.md)** | Static security & quality analysis gate (Bandit + magic-number qua Ruff) | ✅ Hoàn thành (4/4 task con) |
...
| **[EPIC-007](epics/EPIC-007_chuan_hoa_card_dung_chung/README.md)** | **Chuẩn hoá card dùng chung, đưa hình dạng lên Engine** — gộp ~10 biến thể màu card về 1 token, 6 hình dạng surface lên `pyside_mvc.widgets`, cắt 3 import chéo màn hình | ✅ **Hoàn thành (8/8 task con, `007A`–`007H`)** — 2026-08-26 |
...
| **[EPIC-014](epics/EPIC-014_widget_chon_symbol_va_timeframe_dung_chung/README.md)** | **Widget chọn Symbol & Timeframe dùng chung** — ba bản picker symbol khác nhau (Data Management mỏng, Backtest bản sao riêng, Dev Board chỉ có `QComboBox` nhét cứng 2 mã); picker timeframe chỉ cho chọn 5/16 khung `TimeFrame` vì đang đọc `DEFAULT_TIMEFRAMES` — một tuple canh hàng pill của chart toolbar. Thêm yêu thích/gần đây dùng chung mọi màn hình (`EPIC-010` ui_state). Đã gộp cả 4 màn (Backtest, Dev Board, Data Management, Settings) + `ChartToolbar`; xoá 3 file picker cũ | ✅ **Hoàn thành** — 2026-08-28 |
| **[EPIC-015](epics/EPIC-015_qml_tung_widget_khong_chuyen_chart/README.md)** | **QML từng widget, chart ở lại QtWidgets** — QML lồng được vào QtWidgets, chiều ngược lại thì không, nên chart pyqtgraph (4.253 dòng) và shell ở lại widget, mọi thứ khác chuyển dần: modal → Settings → Data Management → Dev Board → Backtest. Widget ở DIR riêng, ViewModel của widget test riêng không cần GUI (đã đo) | ✅ **Hoàn thành cả 4 phase** — 2026-08-30. `SymbolPicker`/`TimeRangePicker`/`TimeframePicker` (modal)/`KlineInspectorTable` (Phase 1) + `DatabaseStatusTable`/`ProgressBanner` ở Data Management (Phase 2) + `MetricsDetailPanel` thay `ExtendedMetricsDialog`/`StatGrid` ở Backtest (Phase 3) + `TimeframeToolbar`/`StatusPill`/`StatCard`+`ProgressBanner` cạnh chart thật (Phase 4, **user đã xác nhận bằng mắt chart không giật/không rớt khung hình**). Shell + chart pyqtgraph ở lại QtWidgets vĩnh viễn, mọi widget khác trong phạm vi epic đã sang QML |
| **[EPIC-016](epics/EPIC-016_screen_registry_pattern/README.md)** | **Screen Registry Pattern** — `MainWindow` hard-code toàn bộ nav + router setup; `ISidebar`/`AbstractScreenModule`/`IScreenRegistry` tách nó ra, không tạo type song song với `ITab`/`SidebarSection` có sẵn | ✅ **Hoàn thành (4/4 task con)** — 2026-08-30 |
| **[EPIC-017](epics/EPIC-017_presentation_hard_design_round2/README.md)** | **Presentation Hard-Design Round 2** — verify độc lập report ngoài: Settings hard-code field 3 màn khác, dùng `TimeFrame` Enum đã có sẵn thay literal `"1m"`; God Presenters route sang `EPIC-003` (`003G`); 2 đề xuất bị từ chối (auto-discovery, bỏ Junction hack) | ✅ **Hoàn thành (2/2 task con)** — 2026-08-30 |
...
| **[EPIC-021](epics/EPIC-021_ket_noi_binance_futures_testnet/README.md)** | **Kết nối Binance USD-M Futures Testnet & đường đi lệnh thật** — app hiện **không có khái niệm môi trường sàn**: DI dựng `PythonBinanceClient` không tham số (`binance_bot_module.py:231`) nên luôn mainnet ẩn danh, key user nhập ở Settings không bao giờ tới client (`BUG-080`), 2 key endpoint trong config là config chết (`BUG-081`), và parser metadata (step/tick/minNotional) chỉ được `tests/` gọi. Chọn **Futures** thay vì Spot vì mô hình khớp lệnh của repo đã là futures (SHORT + leverage trong `PaperExchange`); Settings có **2 lựa chọn độc lập** (nguồn dữ liệu vs nơi đặt lệnh); `TradingVenue` **không có member MAINNET** — rào an toàn bằng type, không bằng cấu hình | ✅ **Xong (13/13 task con + `BOT-124`)** — **A** xong (đóng `BUG-081`), **B**+**D** xong (đóng hẳn `BUG-080`), **C**+**E**+**F**+**G**+**H**+**J**+**L** xong (**L** đóng `BUG-082`); **I** xong — màn hình **Giao dịch** mới (mở `BUG-086`, đóng lại cùng ngày); **K** xong — banner môi trường ở cả 5 màn, Emergency Stop, trade marker trên chart; **M** xong — chart vốn realtime lấy mẫu từ `ACCOUNT_UPDATE`, không request thêm. 7 mốc đầu chạy headless bằng CLI — **G** là lệnh thật đầu tiên, **H** là User Data Stream nối vào `OrderFeed`, **J** là tier `tests/testnet/` opt-in chạm sàn thật |
| **[EPIC-022](epics/EPIC-022_chien_luoc_song_tren_man_giao_dich/README.md)** | **Chiến lược sống trên màn Giao dịch** — user hỏi "sao không có ô chọn chiến lược? vậy nó chạy chiến lược nào mà khớp?"; khảo sát lộ ra thứ tệ hơn thiếu combobox: chiến lược live chọn bằng 1 key config đọc **một lần lúc boot** (`binance_bot_module.py`), giá trị mặc định **rỗng**, nên bật giao dịch xong **không có gì chạy** mà UI vẫn báo "Trading đang BẬT" — vi phạm thẳng `domain-truth-rule.md`. `LiveStrategySession` thay 4 field đóng băng của `MarketTickEventHandler`; `ArmStrategyCommand`/`DisarmStrategyCommand` + `EnableTradingBlockReason.NO_STRATEGY_ARMED`; card CHIẾN LƯỢC + Thông số Chiến lược trên màn Giao dịch; đường chỉ báo + vùng xu hướng của chiến lược vẽ lên chart live **dùng chung đúng 2 hàm với Backtest** (thăng cấp khỏi `screens/backtest/`) nên 2 màn không thể lệch nhau | ✅ **Hoàn thành (6/6 task con)** — 2026-09-07 |
| **[EPIC-023](epics/EPIC-023_dev_board_tinh_nang_co_ban_giao_dich/README.md)** | **Dev Board có các tính năng cơ bản của màn Giao dịch** — theo yêu cầu user; combo "Strategy" trên Dev Board là hàng giả (hard-code, không nối `ArmStrategyCommand`), không có Vị thế/Lệnh chờ/Vốn/Bật-Tắt-giao-dịch/Dừng-khẩn-cấp. User chốt: Dev Board **toàn quyền** điều khiển giống hệt Trading, tái dùng nguyên `StrategyArmingCoordinator`/`OrderFeed`/`EquityFeed`/`SignalFeed` — không sửa file nào của `screens/trading/` | ✅ Hoàn thành (4/4 task con) |
| **[EPIC-024](epics/EPIC-024_modularize_trading_core_va_giao_dich_thu_cong/README.md)** | **Giao dịch thủ công trên Dev Board + Modularize Trading Core** — từ [`PRO-003`](proposal/PRO-003.md): user muốn tự tay đặt Long/Short như sàn thật, qua ĐÚNG `ExecuteOrderCommand` mà chiến lược đang dùng — cơ chế đó có tổng quát cho caller thứ hai (con người) hay không **chưa được chứng minh**, và chính việc xây tính năng thủ công là bước chứng minh, không phải hệ quả của một sự thật đã chứng minh sẵn. Song song, tách bạch Market Connector/Market Order/Strategy Engine thành module tường minh (hiện chỉ là các hàm gộp trong `BinanceBotModule` 656 dòng) — chốt phạm vi *sau* khi tính năng thủ công merge, dựa trên bằng chứng thật, không phải nợ kỹ thuật để "làm sau nếu có thời gian" | 🟡 Đang làm (1/3 task con) — **A** xong: 3 handler đổi sang port mới `ITradingSessionFactory` |
...
- [x] **`BOT-132`**: [`QuickSurface` — một tầng nhúng QML duy nhất, thay 10 host tự dựng `QQuickWidget`. Nguyên nhân gốc của `BUG-115` (user: "Windows tốt, Ubuntu glitch, nghĩ do cơ chế bridge") không phải một dòng sai mà là **cầu nối QtWidgets↔QML chỉ là quy ước copy-paste**: 10 host lặp cùng 12 dòng, kèm `setClearColor(transparent)` với giả định "nền cha sẽ lộ ra" — chỉ đúng trên đường vẽ software (`offscreen`/`grab()`, tức toàn bộ tầng test), còn đường texture của mọi desktop thật thì Qt đục lỗ backing store rồi clear đen. Và Engine **đã có** `create_quick_widget()` nhưng app bỏ qua ở 17 chỗ từ khi `EPIC-006F` gỡ `configure_app_qml()` khỏi bootstrap. Sửa: `qml/embed/QuickSurface(surface: StyleRole)` là **nơi duy nhất** dựng `QQuickWidget` (qua factory Engine, `TASK-042`), nền đục đọc từ **cùng** bảng token `apply_role()` vẽ nền cha; `kit/style.py::background_token(role)`; guard cấm dựng `QQuickWidget`/`setClearColor(` ngoài `qml/embed/`; tầng Desktop mới `scripts/quick_surface_desktop_probe.py` — tầng đầu tiên của repo chạy đường texture, thứ đã bỏ lọt cả `BUG-102` lẫn `BUG-115`. Đo lại trên X11 thật: pixel-diff `grab()` ↔ màn hình 24,12 % → **0,00 %**](completed/BOT-132_quick_surface_tang_nhung_qml_duy_nhat.md)
- [x] **`BUG-113`**: [Mỗi tick kline live log ở mức INFO — tái diễn đúng lớp lỗi `BUG-042`/`BUG-095`. User đọc log thật của chính mình, chỉ đúng lý do không cần gợi ý trước. `BinanceWebsocketService._process_socket_message()` gọi `logger.info(...)` cho mọi message kline WebSocket — vài dòng mỗi giây trên symbol đang hoạt động. `BUG-095` đã sửa lớp này ở `futures_user_data_stream.py` nhưng dòng `[Live Stream]` sống ở file khác nên lọt lưới. Đổi `logger.info` → `logger.debug`. 1 test mới, xác nhận đỏ đúng lý do trước khi sửa](bug_report/completed/BUG-113_live_stream_kline_tick_logged_at_info.md)
- [x] **`BUG-112`**: ["Bật giao dịch" bắt buộc nạp chiến lược dù chỉ muốn giao dịch thủ công — vòng luẩn quẩn thật. User tự chạy app, mô tả trực tiếp: giao dịch thủ công cần Bật giao dịch, Bật giao dịch cần nạp chiến lược, mà nạp chiến lược đúng symbol muốn giao dịch tay lại bị luật khác (`PRO-003` §4.1.2) chặn tiếp. Root cause: gate `NO_STRATEGY_ARMED` (`EPIC-022B`) viết từ trước khi "Bật giao dịch" mang nghĩa dùng chung cho cả chiến lược tick và người bấm tay (`EPIC-024B`). Bỏ hẳn điều kiện + tham số `strategy_session` khỏi `EnableTradingCommandHandler`, xoá enum chết. 1 test mới, xác nhận đỏ đúng lý do trước khi sửa](bug_report/completed/BUG-112_enable_trading_requires_a_strategy_manual_trading_never_needed.md)
- [x] **`BUG-111`**: [Mọi request có ký (`futures_*`) tới Binance Futures Testnet thất bại vĩnh viễn với `-1021` khi đồng hồ máy chạy nhanh — user báo qua log INFO thật lúc tự chạy app trên Testnet (`GetOpenPositionsQuery failed: ... APIError(code=-1021)`, 2 lần cách nhau 8s, cùng lỗi). Root cause: `python-binance`'s `timestamp_offset` mặc định `0`, `ExchangeSessionFactory.create_trading_client()` chưa từng chỉnh — mọi request ký bằng đồng hồ local trần, mà Binance từ chối cứng nếu timestamp vượt server-time hơn 1000ms (`recvWindow` không nới được ngưỡng này). `FuturesAccountReader.check_connection()` đã đo đúng độ lệch này từ `EPIC-021D` nhưng chỉ để hiển thị, không áp lại. Sửa: đồng bộ `timestamp_offset` qua `futures_time()` ngay tại nơi duy nhất mọi session có ký được tạo, nên mọi consumer tự động được sửa. 1 test mới, xác nhận đỏ đúng lý do trước khi sửa](bug_report/completed/BUG-111_signed_binance_requests_never_sync_clock_skew.md)
- [x] **`BUG-109`**: [Realtime/tick mode + preset "365 ngày qua" (khoảng có giới hạn, không phải "Toàn bộ lịch sử") treo "Chạy Backtest" nhiều phút, không thanh tiến trình — user báo qua log INFO thật. `_coverage_is_ready()` gọi `probe_coverage()` đồng bộ, không progress, dispatch `GetBacktestRangeCoverageQuery` ở interval hiệu lực 1s (tick_resolution, không phải khung hiển thị chart) trên 364 ngày — 52× cửa sổ 7-ngày `BOT-075` đã đo ~17s và kết luận "không dùng được đồng bộ". `TickModeRequiresBoundedRangeRule` (`BUG-073`) chỉ chặn `start_time is None`, chưa từng chặn khoảng có-giới-hạn-nhưng-quá-rộng. Thêm ngưỡng 7 ngày (tái dùng số đã validate ở BOT-075) vào rule đó + `ChartPreviewCoordinator`, 11 test mới. Đổi số từ `BUG-107` — trùng số với `BUG-107` của một phiên song song (mở màn Giao dịch tự live)](bug_report/completed/BUG-109_tick_mode_bounded_wide_range_coverage_probe_hangs.md)
- [x] **`BOT-128`**: [Bảng cuộn ngang thay vì mất cột. Header cell mang `Layout.minimumWidth: 0` (di sản `BUG-076`, vốn chỉ cần cho cột `fillWidth`) khiến **mọi** cột co được xuống 0 → `RowLayout` bóp cái nào bóp được rồi đẩy phần còn lại ra khỏi mép, và vì bề rộng cột là SSOT dùng chung với row delegate (không co) nên header lệch hẳn khỏi row. Đo: bảng Vị thế cần 760px, được cấp 391px → mất 4/7 cột. `DataTable` cộng bề rộng đã khai và cuộn `Flickable` tới chúng — chọn hướng **general** (1 component, 6 bảng) thay vì vá tại chỗ màn Giao dịch, theo nguyên tắc user chốt cùng ngày và đã ghi thành luật ở `ONBOARDING.md` §12.5.1](completed/BOT-128_bang_cuon_ngang_thay_vi_mat_cot.md)
- [x] **`BOT-127`**: [Băng console cao theo nội dung. `PageShell` xếp băng console theo `sizeHint`, mà `sizeHint` đó là **hằng số mặc định của `QListView`** — không liên quan số dòng log, nên console giữ 244px ở cả 3 màn kể cả khi chỉ có 1 dòng. Đổi sang `số dòng × chiều cao dòng`, kẹp sàn 2 / trần 8 dòng; nhánh list **rỗng** phải xử riêng vì `sizeHintForRow(0)` trả -1 và đó đúng là ca đang giữ chỗ vô ích. Trading 244→134, workspace +110px. *(File task viết bổ sung sau khi merge — lỗi quy trình, ghi rõ trong file)*](completed/BOT-127_bang_console_cao_theo_noi_dung.md)
- [x] **`BOT-129`**: [Mọi màn hình cuộn khi tràn thay vì bị bóp dẹp. `setWidgetResizable(True)` chỉ cuộn khi **minimum** của nội dung không vừa, nên một cột card co được bị Qt nén và thanh cuộn không bao giờ hiện — Backtest trông như "màn duy nhất cuộn được" chỉ vì nội dung nó tình cờ có minimum 1256px. Đo: rail Giao dịch muốn 1178px, min 302px, được 647px → bóp mất 531px. `PreferredHeightScrollArea` giữ minimum bằng `sizeHint`; chart vẫn co giãn vì viewport cao hơn thì layout vẫn chia phần dư. *(Đổi số từ `BOT-126` — trùng số với một phiên song song, xem file)*](completed/BOT-129_moi_man_hinh_cuon_thay_vi_bi_bop.md)
- [x] **`BOT-126`**: [`ILiveStreamService` — sở hữu stream theo từng màn thay vì "ai gọi sau cùng thắng". `EPIC-021I` §6.4 đã ghi nhận (không sửa): 1 stream toàn tiến trình, Dev Board và màn Giao dịch mở cùng lúc giành nhau symbol/khung, `stop()` màn này dừng luôn stream màn kia. Thiết kế lại `subscribe`/`release_owner`/`stop_all` đếm tham chiếu theo `(symbol, interval)` × owner, tận dụng `multiplex_socket` sẵn có; đổi luôn public contract `ILiveStreamService`/`StartLiveStreamCommand`/`StopLiveStreamCommand`. Rà thêm phát hiện lỗ hổng đồng dạng `BUG-085` (lọc tick chỉ theo symbol, không theo interval) ở 2 Presenter UI — thiết kế mới làm khả năng "2 interval cùng symbol" thành hiện thực nên vá luôn cùng lượt](completed/BOT-126_live_stream_ownership_per_screen.md)
...
- [x] **`BUG-079`**: [Chiến lược "Long Term Trend Zone" render nền xu hướng thành dải sọc gần đen trong đoạn thị trường đi ngang — `compute_strategy_trend_zones()` không có ngưỡng tối thiểu, mỗi lần `classify_trend_zone()` đảo chiều (gần mỗi nến khi giá dao động quanh EMA) tạo 1 span riêng, hàng chục span 1-nến xanh/đỏ xen kẽ đọc như khối đen. Tái hiện qua đúng code path production, loại trừ giả thuyết marker Buy/Sell Flags. Sửa: thêm `_MIN_ZONE_BARS = 3`, bỏ hẳn zone dưới ngưỡng (đổi số 2 lần: BUG-077 → BUG-078 → BUG-079, mỗi lần trùng với một bug khác cùng số merge trước)](bug_report/completed/BUG-079_choppy_trend_zone_flips_render_as_striped_black_band.md)
...
- [x] **`BUG-073`**: [Backtest chart-preview probe bắn `GetBacktestRangeCoverageQuery` không giới hạn ở tick mode ngay lúc mở màn — trước khi toolbar's time-range preset ổn định — treo tiến trình lúc thoát app (~19s, sau cả "App stopped." đã log). Sửa: `ChartPreviewCoordinator.request_preview()` áp đúng điều kiện `TickModeRequiresBoundedRangeRule` đã dùng cho nút "Run Backtest" để bỏ qua dispatch đó](bug_report/completed/BUG-073_backtest_preview_unbounded_tick_coverage_hangs_shutdown.md)
- [x] **`BUG-065`**: [CI thật (`pytest tests/ -q` tuần tự, không `-n`) crash native (`Segmentation fault`/`Fatal Python error: Aborted`) tại `test_history_pagination_controller.py` — bisect nhị phân xuống đúng 1 test chứng minh crash đơn luồng: `SymbolPickerOverlay` top-level không Qt parent, card nối signal tới lambda đóng lại `self` tạo chu trình tham chiếu Python, chỉ cyclic GC phá được với thứ tự finalize đụng độ Qt C++ parent-child. Sửa bằng `qtbot.addWidget(dialog)` trong test](bug_report/completed/BUG-065_state_coordinator_test_crashes_a_worker_under_full_parallel_load.md)
- [x] **`BUG-074`**: [`app_config.json`'s `log.viewer.enabled: true` rò 1 thread TCP nền sống suốt phần còn lại phiên CI đơn-tiến-trình — đổi default về `false`](bug_report/completed/BUG-074_log_viewer_enabled_by_default_leaks_tcp_worker_thread.md)
...
| ✅ | **[BOT-038](completed/BOT-038_intermittent_segfault_full_ui_integration_suite.md)** | **Segfault ngẫu nhiên khi chạy toàn bộ `tests/integration/presentation/ui/`** | 🟢 **`S (Fast)`** | — | **Đóng 2026-08-25 dạng re-verified/not reproducing** (không phải "đã fix" — không có code nào đổi). 7 lần chạy lại (tuần tự + `-n 6` + tiến trình Sanity song song, đúng tải thật của `ci-local.ps1`) không tái hiện crash nào. Nghi vấn nguyên nhân: `EPIC-006` đã xoá sạch `QQuickWidget`/`QQmlEngine` khỏi app — cơ chế mà bug object-lifetime này dựa vào có thể không còn tồn tại. `-IncludeFlakyUi` giờ là no-op, giữ lại chỉ để lệnh gọi cũ không lỗi. |
...
| ✅ | **[Epic EPIC-002](epics/EPIC-002_static_type_checking_in_local_ci/README.md)** | **Kiểm tra kiểu tĩnh (`mypy`) trong CI cục bộ** | 🟢 **`S (Fast)`** | — | **Hoàn thành (5/5 task con)** — 2026-09-08. Cổng `mypy` sống trong `ci-local.ps1 -Full`; `EPIC-002E` gỡ `sagittarius_engine` khỏi `ignore_missing_imports`; `EPIC-002D` bật `--strict` thật cho 3/4 layer `src/` (`domain`/`application`/`infrastructure`), `presentation/` cố ý để ngoài (false positive `@Property` PySide6, cần quyết định stub/plugin riêng). Chi tiết ở file riêng của epic (`Tasks/epics/README.md`). |
