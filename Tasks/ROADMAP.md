...
- [x] **`BUG-107`**: [Realtime/tick mode + preset "365 ngày qua" (khoảng có giới hạn, không phải "Toàn bộ lịch sử") treo "Chạy Backtest" nhiều phút, không thanh tiến trình — user báo qua log INFO thật. `_coverage_is_ready()` gọi `probe_coverage()` đồng bộ, không progress, dispatch `GetBacktestRangeCoverageQuery` ở interval hiệu lực 1s (tick_resolution, không phải khung hiển thị chart) trên 364 ngày — 52× cửa sổ 7-ngày `BOT-075` đã đo ~17s và kết luận "không dùng được đồng bộ". `TickModeRequiresBoundedRangeRule` (`BUG-073`) chỉ chặn `start_time is None`, chưa từng chặn khoảng có-giới-hạn-nhưng-quá-rộng. Thêm ngưỡng 7 ngày (tái dùng số đã validate ở BOT-075) vào rule đó + `ChartPreviewCoordinator`, 11 test mới](bug_report/completed/BUG-107_tick_mode_bounded_wide_range_coverage_probe_hangs.md)
