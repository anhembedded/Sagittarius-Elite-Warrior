...
2. `contracts/`: `IStrategyCatalog`, `IStrategyEngineFactory` (for `backtesting` in Phase 3), the
   DTOs `StrategyDescriptor` and `ArmedStrategySnapshot`, the events `SignalEmitted`,
   `StrategyArmed` / `StrategyDisarmed`.
3. `strategy` **consumes** `trading.contracts.IOrderSubmission` — never the reverse (Customer /
   Supplier: `trading` is the supplier). The symbol lease replaces the old hard block: `strategy`
   calls `ITradingSession.claim_symbol` on arm and `release_symbol` on disarm. Signal overlays on
