...
**Trạng thái:** ✅ Hoàn thành (5/5) — 2026-09-08. `EPIC-002D` đóng lại theo quyết định của user
sau 3 lượt tăng (`src/domain/`, `src/application/`, `src/infrastructure/` bật `--strict` thật);
`src/presentation/` **cố ý để ngoài** — bị chặn bởi false positive `@Property` PySide6, cần quyết
định stub/plugin riêng, xem `EPIC-002D` §8.
...
| **[EPIC-002D](completed/EPIC-002D_incremental_strictness_rollout.md)** | Lộ trình siết `--strict` dần theo module (giai đoạn sau, không chặn 3 task trên) | ✅ Đóng (2026-09-08) — `src/domain/` + `src/application/` + `src/infrastructure/` bật `--strict`; `presentation/` cố ý để ngoài, xem §8 |
