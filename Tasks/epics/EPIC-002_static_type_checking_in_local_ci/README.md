...
dài hạn, không chặn gì, tự nó không có mốc "xong hẳn") đã bật `--strict` thật cho 3 module —
`src/domain/`, `src/application/`, `src/infrastructure/` — 2026-09-08.
...
| **[EPIC-002D](incomplete/EPIC-002D_incremental_strictness_rollout.md)** | Lộ trình siết `--strict` dần theo module (giai đoạn sau, không chặn 3 task trên) | 🟡 Đang làm — `src/domain/` + `src/application/` + `src/infrastructure/` bật `--strict` (2026-09-08) |
