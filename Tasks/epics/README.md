...
| **[EPIC-024](EPIC-024_modularize_trading_core_va_giao_dich_thu_cong/README.md)** | **Giao dịch thủ công trên Dev Board + Modularize Trading Core** — từ [`PRO-003`](../proposal/PRO-003.md): user quan sát Trading screen hiện chỉ có một nơi gọi cơ chế đặt lệnh (chiến lược tự động), muốn tự tay đặt Long/Short như sàn thật qua ĐÚNG cơ chế đó, không phải một đường đi riêng. Cơ chế đó có tổng quát cho caller thứ hai (con người) hay không **chưa được chứng minh** — (B) xây tính năng thủ công chính là bước chứng minh, không phải hệ quả của một sự thật đã chứng minh sẵn. (C) modularize Market Connector/Market Order/Strategy Engine chốt phạm vi *sau* (B), dựa trên bằng chứng thật (B) bộc lộ — không phải "làm sau nếu có thời gian" | 🟡 Đang làm (1/3 task con) — **A** xong: 3 handler đổi sang port mới `ITradingSessionFactory` |
