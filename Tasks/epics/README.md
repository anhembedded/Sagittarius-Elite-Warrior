...
| **[EPIC-024](EPIC-024_modularize_trading_core_va_giao_dich_thu_cong/README.md)** | **Giao dịch thủ công trên Dev Board + Modularize Trading Core** — từ [`PRO-003`](../proposal/PRO-003.md): user quan sát Trading screen hiện chỉ có một nơi gọi cơ chế đặt lệnh (chiến lược tự động), muốn tự tay đặt Long/Short như sàn thật qua ĐÚNG cơ chế đó, không phải một đường đi riêng. Cơ chế đó có tổng quát cho caller thứ hai (con người) hay không **chưa được chứng minh** — (B) xây tính năng thủ công chính là bước chứng minh, không phải hệ quả của một sự thật đã chứng minh sẵn. (C) modularize Market Connector/Market Order/Strategy Engine chốt phạm vi *sau* (B), dựa trên bằng chứng thật (B) bộc lộ — không phải "làm sau nếu có thời gian" | ✅ Đóng (A, B xong; C huỷ) — **A** xong: 3 handler đổi sang port mới `ITradingSessionFactory` · **B** xong (đóng 2026-09-11, đã khớp lệnh thật trên Testnet, lộ `BUG-112`/`116`/`117`) · **C** huỷ 2026-09-11, hấp thu vào `EPIC-025` → epic **đóng** |
| **[EPIC-025](EPIC-025_module_theo_bounded_context/README.md)** | **Tách app thành module theo bounded context, trên Microkernel `IExtension` của Engine** — từ [`PRO-004`](../proposal/PRO-004.md): user muốn *"redesign lại khá lớn"* vì app chia theo tầng kỹ thuật, không ai sở hữu bounded context (59 tên method trùng Trading ↔ Dev Board, `binance_bot_module.py` 750 dòng, `BUG-112`/`BUG-117` là giá đã trả). 4 bounded context (`strategy` = Core) + 4 support + kernel; module = `IExtension`; Trading/Dev Board = surface (Dev Board có `dev_probe` khám phá API); Walking Skeleton trong Strangler Fig, 6 phase; Engine nhận mechanism (`EPIC-001D`). Kim chỉ nam: [`Docs/HLD/`](../../Docs/HLD/README.md) | 🔴 Backlog — vòng spec 1 duyệt 2026-09-11 (ADR + HLD §1–§6); vòng 2 (schema contribution point, API Engine) trước khi code `025A` |
