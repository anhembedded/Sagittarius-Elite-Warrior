...
- **Dynamic Responsive UI Sizing & Synchronized Table Columns:**
  ...
  - **A table narrower than its own columns scrolls horizontally; it never drops them** (`BOT-128`). Those declared widths are shared with the row delegate, so letting the header compress (the old `Layout.minimumWidth: 0`) misaligns header from rows **and** pushes the columns that did not fit off the edge with no way to reach them — measured on the Trading screen as 4 of 7 columns simply absent, headers cut mid-word. `DataTable.qml` now owns this for every table: it sums the declared widths and scrolls a `Flickable` to them. A new table gets it by using `DataTable`; do not re-solve it per screen, and do not "fix" a cramped table by shrinking its columns.
