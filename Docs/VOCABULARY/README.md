...
| **`module_id`** | The module's public name, stable forever: used in routes, persisted ui_state, event names, owner ids. Renaming is a breaking change with a state migration. | each module | HLD §3.1 | the Python package name (may differ) |
...
| **Symbol lease** | `trading`'s record that one `owner_id` currently holds a symbol; orders from another owner on that symbol are refused. `strategy` claims on arm, releases on disarm; a manual order carries `owner_id="manual"`. Breaks the trading/strategy cycle without `trading` knowing `strategy`. | `trading` | HLD §3.4, SDD-04 | the stream owner id of `IMarketStream` (same idea, different resource) |
| **Owner id** | The string that names who holds a leased resource (a symbol, a market stream, a background action). Stable, human-readable, usually a `module_id`. | the leasing module | HLD §3.4, SDD-06 | — |
...
| **Contribution** | A module's declaration "put my widget in `<surface>.<place>`", made once in `module.contribute()` as a frozen `ContributionDescriptor` (`module_id`, `surface_id`, `place`, `order`, `size_hint`, `title`, `factory`). Validated when contributed, rendered later. | the contributing module | HLD §4.3, SDD-01 | a widget instance |
...
| **Order** | An integer sorting contributions within one `(surface, place)`; collisions fail at contribute time. | the contributing module | SDD | — |
...
| **Developer mode** (`dev.mode`) | A configuration key read **once at boot**; when true, the `dev_board` surface and every `DEV_PROBE` exist. Toggled on the Welcome screen, written to `user_config.json`, applied by restart. `--dev` wins for the current run. | shell | ADR D14, SDD-05 | a runtime switch |
