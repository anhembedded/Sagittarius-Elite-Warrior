...
| # | File | Specifies |
| :-: | :--- | :--- |
| SDD-01 | [`diagrams/sdd-01_module_contract_classes.puml`](diagrams/sdd-01_module_contract_classes.puml) | The Phase 0 classes: `BoundedContextModule` on the Engine's `IExtension`; `IContributionRegistry`, `ContributionDescriptor`, `ScreenContribution`, `Place`, `SizeHint`; the shell's `ModuleList`, `ContributionRegistry`, `Surface`, `DoubleClaimCheck`; `MarketDataModule` as the first subclass |
| SDD-02 | [`diagrams/sdd-02_boot_sequence.puml`](diagrams/sdd-02_boot_sequence.puml) | Boot, step by step: config read once, `QApplication` before `boot()`, `register()` with no `resolve()`, the double-claim check before `boot()`, `contribute()` / `subscribe()` after boot, the `dev.mode` gate, the first surface |
| SDD-03 | [`diagrams/sdd-03_contribution_render.puml`](diagrams/sdd-03_contribution_render.puml) | How a surface renders a place; one factory contributed to two surfaces yields two widgets sharing one trading-owned coordinator |
| SDD-04 | [`diagrams/sdd-04_order_flow_across_modules.puml`](diagrams/sdd-04_order_flow_across_modules.puml) | A tick becoming an order across `market_data → strategy → trading → gateway → exchange → trading feed → surfaces`, and the symbol lease refusing a manual order |
| SDD-05 | [`diagrams/sdd-05_dev_mode_state.puml`](diagrams/sdd-05_dev_mode_state.puml) | `dev.mode` read once at boot; the Welcome switch writes `user_config.json`; restart applies it (ADR D14) |
| SDD-06 | [`diagrams/sdd-06_market_data_contracts.puml`](diagrams/sdd-06_market_data_contracts.puml) | The Walking Skeleton's public surface: five ports, two DTOs, two events, and who consumes each |
