...
| `backtesting` | ✅ *Position* is a `_OpenPosition` that is **simulated**, mutated by the app on every tick (`paper_exchange.py`) | ✅ run results, trade log | ✅ changes with the simulation model (fees, matching) | ✅ | ✅ one screen of 12,309 lines that the user runs | ✅ | **Bounded context** |
